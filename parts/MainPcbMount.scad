...
module RPiMount(camOffset = 0) {
    difference() {
        union() {
            ...
            translate([camOffset, 31, (cam_width+1)/2])
            cube([cam_width, 3, cam_width+1], center = true);
            
            chunk_size = cam_width / 10;
            for (i = [1:3])
                translate([camOffset-cam_width/2 + i*2 * chunk_size, 31+4, (cam_width + 4)]) {
                    hull() {
            ...
            for (i = [1:3])
                translate([camOffset-cam_width/2 + (i*2+1) * chunk_size, 31+4, (cam_width + 4)]) {
                    hull() {
            ...
            translate([camOffset, 31+4+3, cam_width/2+1])
            cube([cam_width, 3, cam_width], center = true);
        }
        translate([camOffset-cam_width/2-2, 31+4, (cam_width + 4)])
        rotate([0, 90, 0])
